...
target_root = "/scr/hshi74/projects/robocook/target_shapes/"
device = torch.device("cuda" if torch.cuda.is_available() else "cpu")


def make_target(
    args, target_name, step, state_init, tool_name, tool_model_path, params
):
    print(target_name)
    ...
    letter_root = os.path.join(
        target_root, target_name, f"{str(step).zfill(3)}_{tool_name}"
    )
    ...
    # os.system(f"cp -r {letter_root}/{str(step).zfill(3)}_raw.ply " + \
...
def main():
    ...
    target_dict = {
        "dumpling_gnn": {
            "tool": ["gripper_sym_plane", "press_square", "roller_large"],
            "params": [
                [[0.0, 0.785, 0.015], [0.0, 2.355, 0.015]],
                [[0.0, 0.0, 0.01, 0.0]],
                [
                    [-0.02, 0, 0.012, 0],
                    [0, -0.02, 0.007, 1.57],
    ...
    for target_name, info in target_dict.items():
        # os.system(f'cp -r /scr/hshi74/projects/robocook/target_shapes/alphabet/{target_name}/000 ' + \
        #     f'/scr/hshi74/projects/robocook/target_shapes/alphabet/{target_name}/000')
        ...
        for i in range(len(info["tool"])):
            if i == 0:
                state_path = os.path.join(
                    ...
                    target_name,
                    f"{str(i+1).zfill(3)}_cutter_planar/{str(i+1).zfill(3)}_surf.h5",
                )
            else:
                state_path = os.path.join(
                    target_root.replace("gnn", "real"),
                    target_name,
                    f"{str(i+1).zfill(3)}_{info['tool'][i-1]}/{str(i+1).zfill(3)}_surf.h5",
                )
