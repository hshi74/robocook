...
from ..utils import disable_gradients, soft_update, update_params, assert_action
import pdb
...
class SAC(Algorithm):
    def __init__(
        ...
        log_interval=10,
        GaussianPolicy=None,
        TwinnedStateActionFunction=None,
    ):
        super().__init__(state_dim, action_dim, device, gamma, nstep, log_interval)
    ...
    def explore(self, state, bounds):
        ...
        action = bounds[:, 0] + 0.5 * (action + 1) * (bounds[:, 1] - bounds[:, 0])
        assert_action(action)
        return action

    def exploit(self, state, bounds):
        state = torch.tensor(
            state[None, ...].copy(), dtype=torch.float, device=self._device
        )
        with torch.no_grad():
    ...
    def update_policy_and_entropy(self, batch, writer):
        ...
        if self._learning_steps % self._log_interval == 0:
            writer.add_scalar(
                "loss/policy", policy_loss.detach().item(), self._learning_steps
            )
            writer.add_scalar(
                "loss/entropy", entropy_loss.detach().item(), self._learning_steps
            )
            writer.add_scalar("stats/alpha", self._alpha.item(), self._learning_steps)
            writer.add_scalar(
                "stats/entropy", entropies.detach().mean().item(), self._learning_steps
            )
    ...
    def update_q_functions(self, batch, writer, imp_ws1=None, imp_ws2=None):
        ...
        # Update Q functions.
        q_loss, mean_q1, mean_q2 = self.calc_q_loss(
            curr_qs1, curr_qs2, target_qs, imp_ws1, imp_ws2
        )
    ...
    def calc_q_loss(self, curr_qs1, curr_qs2, target_qs, imp_ws1=None, imp_ws2=None):
