...
def create_linear_network(
    ...
    assert isinstance(input_dim, int) and isinstance(
        output_dim, int
    ), f"{input_dim}:{type(input_dim)}, {output_dim}:{type(output_dim)}"
...
class StateActionFunction(BaseNetwork):
    def __init__(self, state_dim, action_dim, hidden_units=[256, 256]):
        ...
        self.net = create_linear_network(
            input_dim=state_dim + action_dim, output_dim=1, hidden_units=hidden_units
        )
